...
type OwnerStaffEditScreenNavigationProp = StackNavigationProp<RootStackParamList, 'OwnerStaffEdit'>;
type OwnerStaffEditScreenRouteProp = RouteProp<RootStackParamList, 'OwnerStaffEdit'>;

interface Props {
  navigation: OwnerStaffEditScreenNavigationProp;
  route: OwnerStaffEditScreenRouteProp;
}
...
const OwnerStaffEditScreen: React.FC<Props> = ({ navigation, route }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    status: staff.status,
    shift: staff.shift,
    address: staff.address || '123 Đường ABC, Quận 1, TP.HCM',
    emergencyContact: staff.emergencyContact || '0987654321',
    salary: (staff.salary || 8000000).toString(),
  });
  ...
  const handleRoleChange = (role: Staff['role']) => {
    setFormData(prev => ({ ...prev, role }));
  };

  const handleStatusChange = (status: Staff['status']) => {
    setFormData(prev => ({ ...prev, status }));
  };

  const handleShiftChange = (shift: Staff['shift']) => {
  ...
  const handleSave = async () => {
    ...
    try {
      // Update staff using API service
      const updatedStaff = await OwnerStaffApiService.updateStaff(staff.id, {
        name: formData.name,
      ...
      Alert.alert('Lỗi', 'Không thể cập nhật thông tin nhân viên. Vui lòng kiểm tra lại thông tin.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Chỉnh sửa nhân viên</Text>
        <TouchableOpacity
